...
public class FileIOService {
	...
	public static String CONTACT2_FILE_NAME = "contacts2.txt";
	...
	public void writeData(List<Contacts> contactList) {
		...
		try {
			Files.write(Paths.get(CONTACT2_FILE_NAME), empBuffer.toString().getBytes());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public long countEntries() {
		long entries = 0;
		try {
			entries = Files.lines(new File(CONTACT2_FILE_NAME).toPath()).count();
		} catch (IOException e) {
			e.printStackTrace();
		}
		return entries;
